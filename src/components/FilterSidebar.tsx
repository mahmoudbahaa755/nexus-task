...
import { Filter, Calendar, Film, Star, X, ChevronDown } from 'lucide-react';
          ...
          <Collapsible open={typeOpen} onOpenChange={setTypeOpen}>
            ...
            <CollapsibleContent className="space-y-2 mb-6">
              <Select 
                ...
                onValueChange={(value) => onFilterChange('type', value === '_all_types_' ? '' : value)}
              >
                <SelectTrigger>
          ...
          <Collapsible open={yearOpen} onOpenChange={setYearOpen}>
            <CollapsibleTrigger asChild>
              <Button variant="ghost" className="w-full justify-between p-0 h-auto mb-3">
                ...
                <ChevronDown className={cn("h-4 w-4 transition-transform", yearOpen && "rotate-180")} />
              </Button>
            </CollapsibleTrigger>
            <CollapsibleContent className="space-y-2 mb-6">
              <Select 
                value={getSelectValue(filters.year, '_all_years_')} 
                onValueChange={(value) => onFilterChange('year', value === '_all_years_' ? '' : value)}
              >
                <SelectTrigger>
          ...
          <Collapsible open={genreOpen} onOpenChange={setGenreOpen}>
            <CollapsibleTrigger asChild>
              <Button variant="ghost" className="w-full justify-between p-0 h-auto mb-3">
                ...
                <ChevronDown className={cn("h-4 w-4 transition-transform", genreOpen && "rotate-180")} />
              </Button>
            </CollapsibleTrigger>
            <CollapsibleContent className="space-y-2 mb-6">
              <Select 
                value={getSelectValue(filters.genre, '_all_genres_')} 
                onValueChange={(value) => onFilterChange('genre', value === '_all_genres_' ? '' : value)}
              >
                <SelectTrigger>
                ...
                {filters.type && (
                  <div className="flex items-center justify-between">
                    <Badge variant="secondary" className="flex items-center gap-2">
                      Type: {movieTypes.find(t => t.value === filters.type)?.label}
                      <Button
                        variant="ghost"
                        size="icon"
